...
public class WebServicesSource extends SeismicDataSource implements WebServiceConst {
  /** instance counter. */
  ...
  /**
   * Test flag to allow real-time. To use add "-DSWARM_WS_ALLOW_RT=TRUE" to the
   * Java command line.
   */
  private static boolean allowRealtime;
  static {
    typeString = DataSourceType.getShortName(WebServicesSource.class);
    allowRealtime = Boolean
        .valueOf(WebServiceUtils.getProperty(WebServiceUtils.SWARM_WS_PROP_KEY_PREFIX + "ALLOW_RT"));
    if (allowRealtime) {
      LOGGER.debug("WebServicesSource allowing real-time");
    }
  }
  /** Web Services Client. */
  ...
  /**
   * Create a Web Services server source with the same parameters as the specified
   * Web Services server source.
   * 
  ...
  public synchronized HelicorderData getHelicorder(String station, double t1, double t2, GulperListener gl) {
    ...
    if (hd == null || hd.rows() == 0 || (hd.getStartTime() - t1 > 10)) {
      GulperList.INSTANCE.requestGulper(getGulperKey(station), gl, this, station, t1, t2, gulpSize, gulpDelay);
    }
  ...
  /**
   * Either returns the wave successfully or null if the data source could not get
   * the wave.
   * 
   * @param station the station.
   * @param t1      the start time.
  ...
  /**
   * Is this data source active; that is, is new data being added in real-time to
   * this data source?
   * 
  ...
  public void parse(String params) {
    ...
    configString = String.format("%s;%s:" + PARAM_FMT_TEXT, name, typeString, net, sta, loc, chan, gulpSize, gulpDelay,
        wsDataSelectUrl, wsStationUrl);
    client = new WebServicesClient(this, net, sta, loc, chan, wsDataSelectUrl, wsStationUrl);
    /*
     * try { client.getStationClient().fetch(); } catch (Exception e) { // TODO
