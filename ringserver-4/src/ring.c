...
RingInitialize (char *ringfilename, char *streamfilename, int *ringfd)
{
  ...
  if (config.memorymapring && !config.volatilering)
  {
    ...
    /* Pre-fault the hot zone (most-recently-written end of the ring) into the
     * page cache so the first burst of reads after startup does not stall on
     * hard page faults.  Limit to 1/16 of the ring or 32 MiB, whichever is
     * smaller, to avoid exhausting RAM on large ring files at startup. */
#if defined(MADV_WILLNEED)
    {
      uint64_t hot = config.ringsize / 16;
      if (hot > (uint64_t)(32 * 1024 * 1024))
        hot = (uint64_t)(32 * 1024 * 1024);
      if (hot > 0)
        madvise (param.ringbuffer + config.ringsize - hot, hot, MADV_WILLNEED);
    }
#endif
  }
