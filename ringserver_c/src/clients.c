...
/* Reduced for low latency real-time streaming */
#define THROTTLE_STEPPING 5  /* 5 milliseconds (was 50) */
#define THROTTLE_MAXIMUM 10  /* 0.01 second (was 0.5 second) */
            ...
            int bufcount, int no_wsframe)
{
  ...
  for (idx = 0; idx < bufcount; idx++)
  {
    ...
    {
      /* Use MSG_DONTWAIT to avoid blocking all clients if one is slow.
       * Retry up to 3 times with short delay if buffer is full (EAGAIN/EWOULDBLOCK) */
      int retries = 0;
      do {
        nsent = send (cinfo->socket, buffer[idx], buflen[idx], MSG_DONTWAIT);
        
        if (nsent < 0 && (errno == EAGAIN || errno == EWOULDBLOCK) && retries < 3)
        {
